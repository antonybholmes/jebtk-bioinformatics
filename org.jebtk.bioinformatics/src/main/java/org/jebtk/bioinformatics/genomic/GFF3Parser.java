...
public class GFF3Parser extends GeneParser {
		...
		try {
			while ((line = reader.readLine()) != null) {
				...
				case TRANSCRIPT:
					...
					break;
				case EXON:
					Gene exon = addAttributes(GeneType.EXON,
							region,
							attributeMap);
	...
	public static IterMap<String, String> parseAttributes(String attributes) {
		...
		List<String> tokens = Splitter
				.on(TextUtils.SEMI_COLON_DELIMITER_CHAR)
	...
	public static String formatAttributes(String attributes) {
		String ret = attributes.trim()
				.replace("  ", TextUtils.SPACE_DELIMITER)
