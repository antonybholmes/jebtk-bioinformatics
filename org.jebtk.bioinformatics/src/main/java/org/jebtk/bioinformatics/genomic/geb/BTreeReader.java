...
public class BTreeReader extends BinaryReader {
  ...
  public BTreeReader(Path dir, String prefix, Genome genome, Chromosome chr, int window)
      throws IOException {
  ...
  private int binAddressFromBin(int bin) throws IOException {
    ...
    while (search) {
      ...
      //System.err.println("btree " + bin + " " + b + " " + ba + " " + la + " " + ra);
  ...
  private List<Integer> binAddressesFromTree(long address1, long address2) throws IOException {
    ...
    for (int i = 0; i < n; ++i) {
      seek(treeBinAddresses[i]);
      ret[i] = readInt();
      
      if (i==0 || ret[i] == 1) {
        System.err.println("alert " + i + " " +ret[i] + " " + treeBinAddresses[i]);
      }
    }
  ...
  public List<Integer> elementAddresses(Chromosome chr,
      int start,
    ...
    //System.err.println(b1 + " " + b2); // + " " + Arrays.toString(treeBinAddresses));
